    ...
    public class DeBuildPanel : EditorWindow
    {
            ...
            if (build.foldout) {
                ...
                using (new GUILayout.VerticalScope(DeGUI.styles.box.stickyTop)) {
                    ...
                    using (new GUILayout.HorizontalScope()) {
                        ...
                        build.deleteDebugFolder = DeGUILayout.ToggleButton(
                            build.deleteDebugFolder,
                            new GUIContent(
                                "Delete Debug Folder",
                                "If selected, at the end of the build deletes the eventual \"don't ship\" folder generated by IL2CPP for debug purposes"
                            ),
        ...
        DeBuildResult DoBuild(DeBuildPanelData.Build build, bool andPlay = false)
        {
            ...
            if (build.deleteDebugFolder) {
                ...
                if (!Directory.Exists(debugDataFolder)) {
                    debugDataFolder = Path.GetDirectoryName(buildFilePath)
                                      + DeEditorFileUtils.PathSlash
                                      + build.buildName + "_BurstDebugInformation_DoNotShip";
                }
